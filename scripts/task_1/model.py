...
# %%model stacking
# used lr in sklearn here because it has to be passed to stacked model and
# optimized as a whole by GridSearch CV, the ski-learn api enforce estimators
# to inherit from their base models, which would become very messy and irrelevant
# to this project
lr = LinearRegression()
rf = RandomForestRegressor(
    n_estimators=400, random_state=SEED)
# lasso = Lasso(random_state=SEED) #sklearn implementation as reference benchmark
ridge = BayesianRidge()
svr = SVR()
...
param_grid = {
    "rf__min_samples_split": [3, 12, 36],
    "rf__n_estimators": [100, 200, 400],
    "gbdt__n_estimators": [100, 200, 400],
...
# %%RFE, code referenced sklearn doc
...
rfecv = RFECV(estimator=base, step=1, cv=N_FOLD,
...
new_tele.to_csv("19079662D_task1.csv", index=False)
